...
def getClassName(classNos):
    ...
    if isinstance(classNos, (int, np.int64)):
        return class_names[classNos]
    elif isinstance(classNos, (list, np.ndarray)):
        return [class_names[c] for c in classNos]
    else:
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        for rect in rects:
            ...
            ):  # only detect those signs whose height and width >100
                cv2.rectangle(
                    img_bbx,
                    (rect[0], rect[1]),
            ...
            crop_img = crop_img.reshape(
                1, 32, 32, 1
            )  # (1,32,32) after reshape it become (1,32,32,1)
            predictions = model.predict(crop_img)  # make predicion
            classIndex = model.predict_step(crop_img)
        ...
        if probabilityValue > threshold:
            # Extract the class index from the TensorFlow tensor
            classIndex = np.argmax(predictions[0])

            # write class name on the output screen
            print("classIndex type:", type(classIndex))  # Add this line
            print("classIndex value:", classIndex)  # Add this line
            print("The sign is:", getClassName(classIndex))
            cv2.putText(
                img_bbx,
